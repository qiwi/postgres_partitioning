...
declare
  v_table text;
begin
    v_table       = ''' || v_part_pref || ''' || to_char(new.' || v_tab.pm_part_column || ',''' ||
                                     v_tab.pm_part_name_tmpl || ''');
...
');
    ...
    perform partitioning.execute_ddl(v_tab.pm_schema, v_tab.pm_table_name,
                                     'create  trigger ' || v_trg_name || '   before insert   on ' || v_tab_name ||
                                     ' for each row execute procedure  ' || v_fun_name);
