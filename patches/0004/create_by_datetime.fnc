...
begin
  ...
  loop
    ...
    -- date_trunc v_begin_ts using a string pattern of a partition name
    ...
    loop
      ...
      then
        ...
        perform partitioning.execute_ddl(v_list.pm_schema, v_list.pm_table_name,
                                         ...
                                         || ' ( like ' || v_table_name_full || ' including all ) inherits (' || v_table_name_full
                                         || ') tablespace ' || v_list.pm_tablespace);

        -- a full date in iso format which fits the constraint of partition
        ...
        perform partitioning.execute_ddl(v_list.pm_schema, v_list.pm_table_name,
                  ...
                  (' || v_list.pm_part_column || ' >= ''' || v_date_from || ''' and
                   ' || v_list.pm_part_column || ' <  ''' || v_date_to || ''')');
        ...
        v_begin_ts = v_end_ts;
        -- date_trunc v_begin_ts по шаблону строкового формата
        v_partition_postfix = to_char(v_begin_ts, v_list.pm_part_name_tmpl);
        v_begin_ts = to_date(v_partition_postfix, v_list.pm_part_name_tmpl);
        v_end_ts = v_begin_ts + v_list.pm_part_interval;
